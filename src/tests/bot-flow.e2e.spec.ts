...
describe('Bot flow (e2e)', () => {
        ...
        factory: (config: ConfigService, handlers: HandlersService) => {
          const svc = {
            ...
            onModuleInit: async () => {
              ...
              svc.bot.api.config.use(async (prev, method, payload) => {
                if (method === 'getMe') {
                  return {
                    ok: true,
                    result: {
                ...
                // For methods that return a Message object (sendMessage, sendDocument, etc.)
                if (
                  method === 'sendMessage' ||
                  method === 'sendDocument' ||
                  method === 'copyMessage'
  ...
  it('logs trial_signup event when button pressed', async () => {
    ...
    const event = await botEventRepo.findOne({ where: { action: BotAction.TRIAL_SIGNUP } });
  ...
  it('logs video_meeting event when button pressed', async () => {
    ...
    const event = await botEventRepo.findOne({ where: { action: BotAction.VIDEO_MEETING } });
    expect(event).not.toBeNull();
  ...
  it('logs free_guide event when button pressed', async () => {
    ...
    const event = await botEventRepo.findOne({ where: { action: BotAction.FREE_GUIDE } });
    expect(event).not.toBeNull();
  ...
  it('logs ask_question event and clears state when user replies with question', async () => {
    ...
    await telegramService.bot.handleUpdate(makeTextUpdate(777, 'Frank', questionText));

    const user = await userRepo.findOne({ where: { telegramId: 777 } });
    expect(user).not.toBeNull();

    const event = await botEventRepo.findOne({ where: { action: BotAction.ASK_QUESTION } });
    expect(event).not.toBeNull();
